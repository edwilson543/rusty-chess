...
fn black_cannot_move_pawn_pinned_by_white_queen() {
    ...
    let invalid_move = game.play_move(&Colour::Black, &from_square, &captured_piece_square);
...
fn black_king_cannot_move_to_square_attacked_by_white_knight() {
    ...
    let invalid_move = game.play_move(&Colour::Black, &from_square, &attacked_square);
